...
const getConfig = async ({ basePath, configPath, config: userConfig }) => {
  ...
  try {
    ...
    finalBasePath = res.filepath
  } catch (err) {
    // noop
  }

  mergeConfig([finalConfig, finalFilePath, finalBasePath])
  normalizeConfig([finalConfig, finalFilePath, finalBasePath])
  validateConfig([finalConfig, finalFilePath, finalBasePath])

  return loadTemplate([finalConfig, finalFilePath, finalBasePath])
}
